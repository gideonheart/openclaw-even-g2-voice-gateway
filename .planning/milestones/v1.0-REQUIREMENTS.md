...
- [x] **PIPE-07**: User can switch STT provider via settings API without code changes or restart — fully delivered (runtime provider re-initialization via ConfigStore.onChange(), quick-3 + quick-5)
...
| PIPE-07 | Phase 3 | Complete | `d5df520` Runtime provider re-initialization via ConfigStore.onChange() |
