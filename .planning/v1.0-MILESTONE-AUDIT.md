...
gaps:
  ...
  integration:
    - id: "CLAW-01"
      issue: "Stale shutdown closure in index.ts:103 — calls original openclawClient.disconnect() instead of deps.openclawClient.disconnect() after hot-reload"
      severity: "minor"
      fix: "Change openclawClient.disconnect() → deps.openclawClient.disconnect() in shutdown handler"
  flows: []
tech_debt:
...
All 31/31 v1 requirements are satisfied at the code level. The integration checker verified all requirements as WIRED with specific code evidence. 5/5 E2E flows are complete with zero breaks. One minor code defect found (stale shutdown closure). No critical gaps.

The primary tech debt is **verification process coverage**: Phases 1 and 3 were executed before the GSD workflow was established, so they lack formal VERIFICATION.md files. Their requirements have commit-hash evidence in the traceability table and were verified as WIRED by the integration checker.
...
| CLAW-01 | 1 | — | quick-5 ✓ | [x] `a42657b` | WIRED | **satisfied** (minor: stale shutdown closure) |
...
**1. Stale shutdown closure (minor, affects CLAW-01)**

`services/gateway-api/src/index.ts` line 103: `openclawClient.disconnect()` captures the original local variable. After a hot-reload via `registerOpenClawRebuilder`, the live client is `deps.openclawClient` but shutdown still disconnects the original (already-disconnected) client.

**Fix:** `openclawClient.disconnect()` → `deps.openclawClient.disconnect()`

**Impact:** On SIGTERM after a hot-reload, the current WebSocket client doesn't get a clean close frame. It disconnects via process termination instead. Not a data-loss risk; the WebSocket server (OpenClaw) handles abrupt disconnects.
...
| Item | File | Impact |
|------|------|--------|
| Stale shutdown closure after hot-reload | `index.ts:103` | Minor — no clean close frame on SIGTERM post-hot-reload |
...
### Total: 7 items (0 critical, 1 minor defect, 6 informational)
...
_Audited: 2026-02-28T08:30:00Z_
